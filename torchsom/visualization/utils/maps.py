...
def build_bmus_data_map(
    ...
    Args:
        ...
        batch_size (int): Batch processing size (ignored when data is a DataLoader)
    ...
    if isinstance(data, DataLoader):
        global_idx = 0
        for batch in data:
            batch_data = (batch[0] if isinstance(batch, (list, tuple)) else batch).to(som_instance.device)
            ...
            for i, (row, col) in enumerate(batch_bmus):
                ...
                if return_indices:
                    bmus_data_map[bmu_pos].append(global_idx + i)
                else:
...
def build_hit_map(
    ...
    if isinstance(data, DataLoader):
        if batch_size:
            print("batch size is derived from dataloader and ignored!!")
