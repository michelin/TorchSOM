...
class SOM(BaseSOM):
    ...
    def _update_weights(
        ...
        if bmus.dim() != 2 or bmus.shape[1] != 2:
            raise ValueError(
                f"_update_weights: bmus must be [batch, 2], got {tuple(bmus.shape)}; "
    ...
    def fit(
        ...
        test_data: torch.Tensor | DataLoader | None = None,
        epochs: Optional[int] = None,
        snapshots_path: Optional[str|Path] = None,
        return_hitmaps: bool = False,
    ) -> tuple[list[float], list[float]] | tuple[list[float], list[float], np.ndarray, np.ndarray|None]:
        """Train the SOM using batches and track errors.

        Args:
            data (torch.Tensor): input data tensor [batch_size, num_features] or DataLoader
            verbose (bool, optional): Whether to print progress. Defaults to True.
            wandb_log (bool, optional): Whether to log to wandb. Defaults to False.
            test_data (torch.Tensor | DataLoader | None, optional): Test data. Defaults to None.
                    Only relevant for WandB logging, sends also statistics of the test set
            epochs (Optional[int], optional): fit for number of epochs, i.e. for fine tuning after
                    an already fitted model for `self.epochs`.
            snapshots_path (Optional[str|Path], optional): If provided saves the current weights
                    after each epoch in this location.
            return_hitmaps (bool, optional): Whether to return hitmaps after the losses.
        ...
        epoch_q_errors = []
        epoch_t_errors = []
        hit_counts = test_hit_counts = None

        try:
            for epoch in tqdm(
                range(epochs or self.epochs),
                desc="Training SOM",
                unit="epoch",
                disable=not verbose,
            ):
                ...
                batch_q_errors = []
                batch_t_errors = []
                hit_counts = torch.zeros(self.x * self.y, dtype=torch.long)
                for batch in dataloader:
                    batch_data = (batch[0] if isinstance(batch, (list, tuple)) else batch).to(self.device)
                    # Rerieve the BMUs [batch_size, 2]
                    bmus = self.identify_bmus(batch_data)
                    neuron_idx = (bmus[:, 0] * self.y + bmus[:, 1]).cpu()
                    hit_counts.scatter_add_(0, neuron_idx, torch.ones(neuron_idx.shape[0], dtype=torch.long))
                    # Update the weights
                    ...
                    if _wandb.run is not None:
                        _wandb.log({
                            "batch/quantization_error": batch_q_errors[-1],
                            "batch/topographic_error": batch_t_errors[-1],
                        })

                # In case something fails, we can restart from the last epoch weights
                if snapshots_path is not None:
                    try:
                        torch.save(self.weights, str(Path(snapshots_path) / f"weight_epoch_{epoch}.pt"))
                    except Exception as e:
                        print("Failed writing epoch weights snapshot", snapshots_path, epoch, e)
                ...
                if _wandb.run is not None:
                    # Mean minimum inter-prototype distance
                    # - Computed on the K prototype weight vectors (self.weights), not on data.
                    # - For each prototype, find its nearest neighbour prototype distance; average over all K.
                    # - Higher = prototypes are more spread apart = more distinguishable from each other.
                    # - A value near zero means many prototypes have collapsed onto the same point (dead neurons).
                    # - Comparable across epochs on the same map; not directly comparable across map sizes.
                    w = self.weights.data.detach().view(-1, self.num_features)   # [K, features]
                    proto_dists = torch.cdist(w, w)                              # [K, K]
                    proto_dists.fill_diagonal_(float('inf'))
                    proto_sep = proto_dists.min(dim=1).values.mean().item()
                    hits_f = hit_counts.float()
                    hitmap_cv = (hits_f.std() / hits_f.mean()).item()
                    hitmap_max_ratio = (hits_f.max() / hits_f.mean()).item()
                    _wandb.log({
                        "train/quantization_error": epoch_q_errors[-1],
                        "train/topographic_error": epoch_t_errors[-1],
                        "train/prototype_separation": proto_sep,
                        "train/hitmap_cv": hitmap_cv,
                        "train/hitmap_max_ratio": hitmap_max_ratio,
                        "epoch/nr": epoch,
                        "epoch/minutes": (datetime.now() - start).seconds / 60,
                    })

                    if test_data is not None:
                        test_q_errs, test_t_errs = [], []
                        test_hit_counts = torch.zeros(self.x * self.y, dtype=torch.long)
                        for test_batch in test_dataloader:
                            tb = (test_batch[0] if isinstance(test_batch, (list, tuple)) else test_batch).to(self.device)
                            test_q_errs.append(self.quantization_error(tb))
                            test_t_errs.append(self.topographic_error(tb))
                            t_bmus = self.identify_bmus(tb)
                            t_idx = (t_bmus[:, 0] * self.y + t_bmus[:, 1]).cpu()
                            test_hit_counts.scatter_add_(0, t_idx, torch.ones(t_idx.shape[0], dtype=torch.long))

                        test_q = torch.tensor(test_q_errs).mean().item()
                        test_t = 100 * torch.tensor(test_t_errs).mean().item()
                        test_hits_f = test_hit_counts.float()
                        test_hitmap_cv = (test_hits_f.std() / test_hits_f.mean()).item()
                        test_hitmap_max_ratio = (test_hits_f.max() / test_hits_f.mean()).item()

                        _wandb.log({
                            "test/quantization_error": test_q,
                            "test/topographic_error": test_t,
                            "test/hitmap_cv": test_hitmap_cv,
                            "test/hitmap_max_ratio": test_hitmap_max_ratio,
                            "epoch": epoch,
                        })
                ...
                if epoch < 3:
                    if epoch <= 0:
                        print("""
                            A spike worth worrying about:
                            - Q-err increases between epoch 1 and epoch 2 (means LR is too high, map is oscillating)
                            - Q-err plateau >0.15 after epoch 3 (map not organizing — sigma may be too small or data needs rescaling)
                            - Min and max diverging while mean stays flat (a few neurons winning all BMU competitions — dead neuron problem)
                        """)

                    print(
                        f"Epoch {epoch + 1}: q-err, min, mean, max:\n",
                        torch.tensor(batch_q_errors).min().item(), "\n",
                        torch.tensor(batch_q_errors).mean().item(), "\n",
                        torch.tensor(batch_q_errors).max().item(), "\n",
                    )
        except Exception as e:
            # Log error to w and b and mark the run as failed (any non-zero code works)
            _wandb.log({"crash_error": str(e)})
        ...
        if return_hitmaps:
            return (
                epoch_q_errors,
                epoch_t_errors,
                hit_counts.cpu().numpy() if hit_counts else np.empty([]),
                test_hit_counts.cpu().numpy() if test_hit_counts else None,
            )
        else:
            return epoch_q_errors, epoch_t_errors

    def save(self, path: str) -> None:
        """Save the SOM to a file, including weights and all configuration needed to reconstruct it.

        Args:
            path (str): File path to save to (e.g. "som.pt")
        """
        from torchsom.utils.decay import DECAY_FUNCTIONS  # noqa: PLC0415
        ...
        torch.save({
            "config": {
                ...
                "topology": self.topology,
                "distance_function": self.distance_fn_name,
                "neighborhood_function": self.neighborhood_fn_name,
                "initialization_mode": self.initialization_mode,
                "device": self.device,
                "random_seed": self.random_seed,
                "lr_decay_function": next(k for k, v in DECAY_FUNCTIONS.items() if v is self.lr_decay_fn),
                "sigma_decay_function": next(k for k, v in DECAY_FUNCTIONS.items() if v is self.sigma_decay_fn),
            },
            "state_dict": self.state_dict(),
        }, path)

    @classmethod
    def load(cls, path: str, device: str | None = None) -> "SOM":
        """Load a SOM from a file saved with :meth:`save`.

        Args:
            path (str): File path to load from
            device (str | None): Override the saved device (e.g. "cpu" to load a GPU model on CPU).
                                 Defaults to None (use device stored in checkpoint).

        Returns:
            SOM: Reconstructed and ready-to-use SOM instance
        """
        ...
        som = cls(**config)
        som.load_state_dict(checkpoint["state_dict"])
        return som
    ...
    def build_map(
        self,
        map_type: str,
        data: Optional[torch.Tensor | DataLoader] = None,
        ...
        if map_type in data_dependent_maps:
            ...
            if map_type == "score":
                if "total_samples" not in kwargs:
                    if data is not None:
                        total_samples = len(data.dataset) if isinstance(data, DataLoader) else len(data)
                    else:
                        total_samples = sum(
