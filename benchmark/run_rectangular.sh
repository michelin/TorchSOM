#!/usr/bin/env bash
# Rectangular-topology benchmark sweep over 25x15 (all 9 datasets) and 90x70 (3 widest).
#
...
#
# Companion to run_hexagonal.sh; both share an identical body and differ only in the CONFIG
# block below. Each size gets its own sweep folder suffixed with <topology>_<size>, since the
# per-run results path encodes <topology> but NOT the map size. This supersedes run_all.sh
# for the rectangular re-run (it already covers 25x15), so do not also run run_all.sh.
#
...
# ===== CONFIG (the only block that differs from run_hexagonal.sh) =====================
TOPOLOGY="rectangular"
