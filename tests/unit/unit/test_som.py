...
class TestSOMTraining:
    ...
    def test_fit_wandb_log_false_skips_import(
        self,
        som_small: SOM,
        small_random_data: torch.Tensor,
    ) -> None:
        """Test that wandb_log=False never touches the wandb module."""
        import sys
        data = small_random_data.to(som_small.device)
        wandb_before = "wandb" in sys.modules
        som_small.fit(data, wandb_log=False, verbose=False)
        wandb_after = "wandb" in sys.modules
        # wandb should not have been newly imported by fit()
        assert wandb_before == wandb_after

    def test_fit_wandb_log_true_calls_log(
        self,
        som_small: SOM,
        small_random_data: torch.Tensor,
    ) -> None:
        """Test that wandb_log=True logs the expected metrics each epoch."""
        ...
        with patch.dict("sys.modules", {"wandb": mock_wandb}):
            som_small.fit(data, wandb_log=True, verbose=False)
        assert mock_wandb.log.call_count == som_small.epochs
        first_call_kwargs = mock_wandb.log.call_args_list[0][0][0]
        assert "train/quantization_error" in first_call_kwargs
        assert "train/topographic_error" in first_call_kwargs
        assert "train/prototype_silhouette_score" in first_call_kwargs
        assert "epoch" in first_call_kwargs
